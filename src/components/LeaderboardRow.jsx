  ...
  return (
    <View style={[r.row, { backgroundColor: isMe ? `${colors.coral}14` : colors.card, borderColor: isMe ? colors.coral : colors.glassBorder }]}>
      ...
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={[r.name, { color: colors.text }]} numberOfLines={1}>{name}{isMe ? ' (you)' : ''}</Text>
        <Text style={[r.dept, { color: colors.sub }]} numberOfLines={1}>{department} · {kudosCount} kudos</Text>
      </View>
      <Text style={[r.points, { color: colors.coral }]}>{points} pts</Text>
    </View>
  )
}
